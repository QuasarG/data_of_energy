...
def process_grib_directory(input_dir, output_dir):
    """
    处理目录中的所有GRIB文件

    参数:
        input_dir (str): 包含GRIB文件的输入目录
        output_dir (str): 输出NetCDF文件的目录
    """
    # 获取目录中所有grib文件
    grib_files = [f for f in os.listdir(input_dir)
    ...
    # 逐个处理文件
    for fname in grib_files:
        input_path = os.path.join(input_dir, fname)
        try:
            print(f"\n正在处理文件: {fname}")
            calculate_wind_speed_with_pygrib(input_path, output_dir)
        except Exception as e:
            print(f"处理 {fname} 失败: {str(e)}")
            continue


input_directory = r"M:\era5"
output_directory = r"E:\PythonProjiects\Data_of_energy_competition\wind_speed"
process_grib_directory(input_directory, output_directory)
